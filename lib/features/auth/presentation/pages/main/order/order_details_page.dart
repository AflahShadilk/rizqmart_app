...
class _OrderDetailsPageState extends State<OrderDetailsPage> {
  ...
  void initState() {
    ...
    final random = Random();
    final names = ['Rahul Kumar', 'Amit Singh', 'Vikram Patel', 'Suresh Reddy', 'Mohd. Ali'];
  ...
  Widget _buildTrackingStepper(BuildContext context) {
    ...
    else if (status.contains('out')) currentStep = 3;
    ...
    return Column(
       crossAxisAlignment: CrossAxisAlignment.start,
       children: [
         Text('Track Order', style: context.ts.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
         16.h,
         Row(
           mainAxisAlignment: MainAxisAlignment.spaceBetween,
                 ...
                 children: [
                   Row(
                     children: [
                       Expanded(
                         child: Container(
                           height: 2, 
                           color: index == 0 ? Colors.transparent : (index <= currentStep ? context.cs.primary : context.cs.outlineVariant),
                         ),
                       ),
                       Container(
                         width: 24,
                         ...
                         decoration: BoxDecoration(
                           ...
                           border: Border.all(
                             color: isCompleted ? context.cs.primary : context.cs.outlineVariant,
                             width: 2
                           ),
                         ),
                         child: isCompleted 
                       ...
                       Expanded(
                         child: Container(
                           height: 2, 
                           color: isLast ? Colors.transparent : (index < currentStep ? context.cs.primary : context.cs.outlineVariant),
                         ),
                       ),
                     ],
                   ),
                   ...
                   Text(
                     ...
                     style: context.ts.labelSmall?.copyWith(
                       color: isCompleted ? context.cs.primary : context.cs.onSurface.withAlpha(128), // 0.5
  ...
  Widget _buildDeliveryBoySection(BuildContext context) {
    if (widget.order.status.toLowerCase() == 'pending' || widget.order.status.toLowerCase() == 'cancelled') {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Delivery Partner', style: context.ts.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
        12.h,
        Container(
          ...
          decoration: BoxDecoration(
            color: context.cs.surfaceContainerHighest.withAlpha(77), // 0.3
            borderRadius: BorderRadius.circular(12),
            border: Border.all(color: context.cs.outlineVariant.withAlpha(128)), // 0.5
          ),
          child: Row(
            children: [
              CircleAvatar(
                ...
                child: Text(
                  _deliveryBoyName[0], 
                  style: TextStyle(color: context.cs.primary, fontWeight: FontWeight.bold)
                ),
              ),
              12.w,
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(_deliveryBoyName, style: context.ts.titleMedium?.copyWith(fontWeight: FontWeight.bold, fontSize: 16)),
                    Text('Flutter Express • 4.8 ★', style: context.ts.bodySmall),
                  ],
                ),
              ),
              IconButton(
  ...
  Widget _buildOrderItemsList(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Items', style: context.ts.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
        12.h,
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(item.name, style: context.ts.bodyMedium?.copyWith(fontWeight: FontWeight.bold)),
  ...
  Widget _buildOrderSummary(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Order Summary', style: context.ts.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
        12.h,
         _summaryRow(context, 'Subtotal', widget.order.subtotal),
         8.h,
         _summaryRow(context, 'Delivery Fee', widget.order.deliveryFee),
         8.h,
         _summaryRow(context, 'Discount', -widget.order.discount, isDiscount: true),
         12.h,
         ...
         Row(
           mainAxisAlignment: MainAxisAlignment.spaceBetween,
           children: [
             Text('Total Amount', style: context.ts.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
             Text(
               '₹${widget.order.totalCost.toStringAsFixed(2)}',
               style: context.ts.titleLarge?.copyWith(fontWeight: FontWeight.bold, color: context.cs.primary),
             ),
           ],
         ),
      ],
          ...
          SizedBox(
            ...
            child: OutlinedButton(
               ...
               style: OutlinedButton.styleFrom(
                 ...
                 padding: const EdgeInsets.symmetric(vertical: 16), // Match MainButton height
        ...
        InkWell(
          ...
          child: Padding(
            ...
            child: Row(
              ...
              children: [
                Icon(Icons.headset_mic_outlined, size: 20, color: context.cs.primary),
                8.w,
                Text(
  ...
  void _showCancelDialog(BuildContext context) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text('Are you sure you want to cancel this order? This action cannot be undone and the amount will be refunded to your wallet.'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('No')),
          TextButton(
            onPressed: () {
              Navigator.pop(ctx);
              context.read<OrderBloc>().add(CancelOrderEvent(widget.order.orderId));
      ...
      pw.MultiPage(
        ...
        build: (pw.Context context) {
          return [
            ...
            pw.Row(
              ...
              children: [
                pw.Column(
                  ...
                  children: [
                    pw.Text('RizqMart', style: pw.TextStyle(fontSize: 24, fontWeight: pw.FontWeight.bold, color: PdfColors.blue)),
                    pw.Text('Your Daily Needs, Delivered.', style: const pw.TextStyle(fontSize: 10, color: PdfColors.grey600)),
                  ],
                ),
                pw.Column(
                  ...
                  children: [
                    ...
                    pw.Text('Order ID: #${widget.order.orderId.substring(0, 8).toUpperCase()}'),
                  ],
                ),
              ],
            ),
              ...
              children: [
                pw.Column(
                  ...
                  children: [
                    ...
                    if (widget.order.userPhone != null) pw.Text(widget.order.userPhone!),
                  ],
                ),
                pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.end,
                  children: [
                    ...
                    pw.SizedBox(
                      width: 200,
                      child: pw.Text(widget.order.deliveryAddress ?? '', textAlign: pw.TextAlign.right),
                    ),
                  ],
                ),
              ],
            ...
            pw.Row(
              ...
              children: [
                pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.end,
                  children: [
                    pw.Text('Subtotal:   INR ${widget.order.subtotal.toStringAsFixed(2)}'),
                    pw.Text('Delivery Fee:   INR ${widget.order.deliveryFee.toStringAsFixed(2)}'),
                    pw.Text('Discount:   - INR ${widget.order.discount.toStringAsFixed(2)}', style: const pw.TextStyle(color: PdfColors.green)),
                    pw.Divider(),
                    pw.Text(
            ...
            pw.Center(
               child: pw.Text('Thank you for shopping with RizqMart!', style: const pw.TextStyle(color: PdfColors.grey600)),
            ),
            pw.Center(
               child: pw.Text('For support: support@rizqmart.com', style: const pw.TextStyle(fontSize: 10, color: PdfColors.grey500)),
            ),
          ];
        },
      ),
